...
On my machine, the below code takes about two minutes seconds.
...
knitr::kable(
  ...
  caption = "Markers for whether someone had no change from wave 1 through wave 2 or not, per medium"
) %>% 
...
working_file <- 
  ...
  mutate(
    ...
    # music
    music_time = round(music_hours + (music_minutes / 60), digits = 1),
    
    # films
    films_time = round(films_hours + (films_minutes / 60), digits = 1),
    
    # tv
    tv_time = round(tv_hours + (tv_minutes / 60), digits = 1),
    
    # video games
    games_time = round(games_hours + (games_minutes / 60), digits = 1),
    
    # ebooks
    books_time = round(books_hours + (books_minutes / 60), digits = 1),
    
    # magazines
    magazines_time = round(magazines_hours + (magazines_minutes / 60), digits = 1),
    
    # audiobooks
    audiobooks_time = round(audiobooks_hours + (audiobooks_minutes / 60), digits = 1)
  )
```
...
We decided to only impute an entire wave if a person has at least fives waves.
...
**Overall, `r round((nrow(exclusions_wave) / nrow(working_file)), digits = 2) * 100`% of rows are excluded.**
...
Both the `estimates` and the `identity` items had strongest agreement as 1.
...
And the well-being scales per wave.
```{r build-well-being-scales}
working_file <- 
  working_file %>% 
  mutate(
    well_being = rowMeans(select(., starts_with("well_being"), starts_with("life_satisfaction")))
  )
```
...
And then remove those who don't have at least five waves.
