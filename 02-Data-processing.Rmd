...
Currently, I'm allowed to share the raw data, which is why they're not on the Github repo yet.
...
knitr::kable(
  example_ids,
  caption = "Example of how ID variables look"
)
```
...
On my machine, the below code takes about 30 seconds.
```{r assign-pp-id}
...
# we can do this easily with coalesce, for which we create a vector that continues from the current max ID until
# each participant with missing ID has one
working_file <- 
  working_file %>% 
  mutate(
    id = coalesce(
...
working_file <-
  ...
  select(
    ...
    # demographics
    occupation_main_earner = PS1a,
    main_earner = PS1b,
    income_main_earner = PS1c,
    income_own = A7,
    ethnicity = PS2,
    internet_use = A2,
    gender = A4,
    age = A5,
    employment = A6x1,
    living_on_my_own = A6x2r1,
    living_with_children = A6x2r2,
    living_with_partner = A6x2r3,
    living_with_partner_children = A6x2r4,
    living_with_family = A6x2r5,
    living_with_parents = A6x2r6,
    living_with_friends = A6x2r7,,
    region = A8,
    covid_work_home_often = A6x4r1,
    covid_work_home_always = A6x4r2,
    covid_work_outside = A6x4r3,
    covid_work_stopped = A6x4r4,
    covid_home_schooling = A6x4r5,
    covid_self_isolating = A6x4r6,
    covid_none = A6x4r99,
...
Alright, now to the tedious part: transforming all variables to the correct type and assigning informative factor level labels.
...
working_file <- 
  ...
  mutate(
    across(
      c(
        id,
        occupation_main_earner:gender,
        employment:covid_none
      ),
      as.factor
    )
  ) %>% 
  ...
  mutate(
    occupation_main_earner = fct_recode(
      occupation_main_earner,
      "Higher managerial/professional/administrative" = "1",
      "Intermediate managerial/professional/administrative" = "2",
      "Supervisory or clerical/junior managerial/ professional/ administrative" = "3",
      "Skilled manual worker" = "4",
      "Semi-skilled or unskilled work" = "5",
      "In full time education" = "6",
      "Unemployed" = "7",
      "Prefer not to say" = "8"
    ),
    across(
      c(income_main_earner, income_own),
      ~ fct_recode(
        .x,
        "Less than £10,000" = "1",
        "£10,000 - £19,999" = "2",
    ...
    ethnicity = fct_recode(
      ...
      "Chinese" = "7",
      "Japanese" = "8",
      "Any other Asian/Asian British Origin" = "9",
      "African" = "10",
      "Caribbean" = "11",
      "Any other Black/African/Caribbean British origin" = "12",
      "Arab" = "13",
      "Any other ethnic origin" = "14",
      "White and Black Caribbean" = "15",
      "White and Black African" = "16",
      "White and Asian" = "17",
      "Any other Mixed/Multiple ethnic origins" = "18",
    ...
    employment = fct_recode(
      ...
      "Employed but furloughed due to the coronavirus pandemic" = "3",
    ...
    # assign yes/no to binary variables
    across(
      c(
        starts_with("living"),
        starts_with("covid"),
...
Note that I check at the beginning of the code chunk whether any of those filter variables have missing values, but it seems the survey had forced responses here, so we don't have missings.
...
filters <- 
  ...
  select(
    id,
    occupation_main_earner:internet_use,
    contains("identity"),
    starts_with("filter")
  )
...
First, let's see how many people completed each wave, what percentage of people have completed that exact number of wave, how many participants that have exactly this number of waves, and what percentage of participants have completed *at least* this wave.
```{r check-wave-completes}
completion_table <- 
  working_file %>% 
  count(id) %>% 
  count(n) %>% 
...
Here's why that can be problematic: The participants below hadn't listened to audio books in the first three months and reports that at wave 1, then says at wave 2 that their audio book use hadn't changed (i.e., selected `3` on the `audiobooks_estimate` question).
But the participant was still asked about their minutes and hours - which is why they filled in bogus numbers.
Minutes and hours were force response as far as I can tell.
...
Therefore, if someone says that they didn't use a medium in the three months before the first wave, they are saying that they *spent zero amount of time engaging with that medium*.
...
Below, I'll set all time variables at the first wave to zero that were skipped in the survey because participants said they didn't use the medium in the filter questions.
...
working_file <- 
  ...
  mutate(
    ...
    across(
      ...
      ~ if_else((wave == 1 & filter_music == 0), 0, .x)
    ),
    
    # films
    across(
      c(films_hours, films_minutes),
      ~ if_else((wave == 1 & filter_films == 0), 0, .x)
    ),
    
    # tv
    across(
      c(tv_hours, tv_minutes),
      ~ if_else((wave == 1 & filter_tv == 0), 0, .x)
    ),
    
    # video games
    across(
      c(games_hours, games_minutes),
      ~ if_else((wave == 1 & filter_video_games == 0), 0, .x)
    ),
    
    # ebooks
    across(
      c(books_hours, books_minutes),
      ~ if_else((wave == 1 & filter_ebooks == 0), 0, .x)
    ),
    
    # magazines
    across(
      c(magazines_hours, magazines_minutes),
      ~ if_else((wave == 1 & filter_magazines == 0), 0, .x)
    ),
    
    # music
    across(
      c(audiobooks_hours, audiobooks_minutes),
      ~ if_else((wave == 1 & filter_audiobooks == 0), 0, .x)
    )
  )
```
...
When they answered anything but that they didn't use the medium, they were asked to provide minutes and hours estimates.
...
If participants indicated on one of the `estimate` questions that they didn't use that medium in the previous week (only applicable in waves 2 to 6), they'll receive an `NA` on their time estimates.
So we'll first set time estimates to 0 if participants say that they didn't use a medium.
```{r didnt-use-medium}
working_file <- 
  working_file %>% 
  mutate(
    ...
    across(
      c(music_hours, music_minutes),
      ~ if_else((wave != 1 & music_estimate == 6), 0, .x)
    ),
    
    # films
    across(
      c(films_hours, films_minutes),
      ~ if_else((wave != 1 & films_estimate == 6), 0, .x)
    ),
    
    # tv
    across(
      c(tv_hours, tv_minutes),
      ~ if_else((wave != 1 & tv_estimate == 6), 0, .x)
    ),
    
    # video games
    across(
      c(games_hours, games_minutes),
      ~ if_else((wave != 1 & games_estimate == 6), 0, .x)
    ),
    
    # ebooks
    across(
      c(books_hours, books_minutes),
      ~ if_else((wave != 1 & books_estimate == 6), 0, .x)
    ),
    
    # magazines
    across(
      c(magazines_hours, magazines_minutes),
      ~ if_else((wave != 1 & magazines_estimate == 6), 0, .x)
    ),
    
    # music
    across(
      c(audiobooks_hours, audiobooks_minutes),
      ~ if_else((wave != 1 & audiobooks_estimate == 6), 0, .x)
    )
  )
```
...
In Table X I display how the data look like after transformation.
```{r media-to-long}
media_long <- 
...
knitr::kable(head(no_changes, n = 10))
```
...
working_file <- 
  ...
  mutate(
    ...
    across(
      c(music_hours, music_minutes),
      ~ if_else((wave != 1 & music_no_change == TRUE), 0, .x, missing = .x) # need to set missing explicitly because the no_change variables contain NA if someone didn't have a missing in the first wave
    ),
    
    # films
    across(
      c(films_hours, films_minutes),
      ~ if_else((wave != 1 & films_no_change == TRUE), 0, .x, missing = .x)
    ),
    
    # tv
    across(
      c(tv_hours, tv_minutes),
      ~ if_else((wave != 1 & tv_no_change == TRUE), 0, .x, missing = .x)
    ),
    
    # video games
    across(
      c(games_hours, games_minutes),
      ~ if_else((wave != 1 & games_no_change == TRUE), 0, .x, missing = .x)
    ),
    
    # ebooks
    across(
      c(books_hours, books_minutes),
      ~ if_else((wave != 1 & books_no_change == TRUE), 0, .x, missing = .x)
    ),
    
    # magazines
    across(
      c(magazines_hours, magazines_minutes),
      ~ if_else((wave != 1 & magazines_no_change == TRUE), 0, .x, missing = .x)
    ),
    
    # music
    across(
      c(audiobooks_hours, audiobooks_minutes),
      ~ if_else((wave != 1 & audiobooks_no_change == TRUE), 0, .x, missing = .x)
    )
  ) %>% 
