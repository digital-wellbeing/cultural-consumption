...
```{r correlation-matrix-estimates-well-being, echo=FALSE, cache=TRUE, message=F, warning=F, fig.cap="Correlation matrix of estimated time spent with a medium and well-being", fig.dim=c(14,14)}
ggpairs(
  data = working_file %>%
    select(
      contains("estimate"),
...
ggsave(
  filename = here("figures", "figure1.tiff"),
  plot = figure1,
  width = 21 * 0.8,
  height = 29.7 * 0.4,
  units = "cm",
...
ggsave(
  filename = here("figures", "figure2.tiff"),
  plot = figure2.1,
  width = 21 * 0.9,
  height = 29.7 * 0.4,
  units = "cm",
...
Next, I'll do the same thing with well-being.
